...
fn skills_list_rendering_does_not_leak_description_text() {
    let (adapter, script) = mock_core(
        r#"read REQUEST
# Match each key separately: serde_json key order depends on whether a
# workspace dependency enables preserve_order, so one glob spanning two keys
...
case "$REQUEST" in
  *'"domain":"skills"'*)
    ...
    ;;
  *) printf '%s\n' '{"success":false,"error":"unexpected domain"}' ;;
esac"#,
    );
    let mut state = en_state();
    let mut buf = Vec::new();
