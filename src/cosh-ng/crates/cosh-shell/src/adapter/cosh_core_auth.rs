...
impl AdapterInstance {
    ...
    ///
    /// Routed through the registry capability rather than `CoshCoreAdapter`:
    /// under ACP the agent serving the turn need not be cosh-core, and a
    /// backend without a registry reports that instead of being skipped.
    pub(crate) fn ai_configured(&self) -> Result<bool, String> {
        let value = self.registry_request("auth", "state", serde_json::Value::Null)?;
