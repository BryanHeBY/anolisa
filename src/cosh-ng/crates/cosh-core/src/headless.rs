...
pub async fn run(
    ...
    // Hold the lock for the whole run instead of reacquiring it per write.
    let stdout = io::stdout();
    let writer = io::BufWriter::new(stdout.lock());
    let lines = BufReader::new(tokio::io::stdin()).lines();
    run_with_io(args, config, project_root, workspace, writer, lines).await
}
    ...
    R: AsyncBufReadExt + Unpin,
{
    ...
    if let Some(ref prompt) = args.prompt {
        ...
        let turn_result = match lease.as_ref() {
            ...
            // No turn ran, so none failed; the lease error reaches the caller
            // through the persist arm below.
            Err(_) => Ok(AgentTurnOutcome::Completed),
        };
    ...
    match msg {
        ...
        InputMessage::User {
            ...
            let turn_result = match lease.as_ref() {
                ...
                // No turn ran, so none failed; the lease error reaches the
                // caller through the persist arm below.
                Err(_) => Ok(AgentTurnOutcome::Completed),
            };
            let persist_result = match lease {
                Ok(()) => {
