...
async fn run() {
    ...
    // ACP serves the same turn engine over stdio, and `is_headless` excludes it
    // by design, so resolve the workspace for either entry point.
    let (project_root, session_workspace) = if agent_headless || args.is_acp() {
    ...
    if let Some(cli::Command::Mcp(mcp)) = args.command {
        ...
        let Some(session_workspace) = session_workspace else {
            eprintln!("[cosh-core] acp workspace was not initialized");
            std::process::exit(2);
        };
        let code = acp::run(&args, config, project_root, session_workspace).await;
        if code != 0 {
